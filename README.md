...
The project is being built in phases. The local synthetic bank app, capability/safety contracts, genuine model-driven discovery, generated capability, and deterministic replay are implemented. Runtime recovery and human handoff come later. See [PLAN.md](PLAN.md), the [agreed demo scope](docs/phase-1-scope.md), the [Phase 3 contract](docs/phase-3-contract.md), the [Phase 4 design](docs/phase-4-discovery.md), and the [Phase 5 replay design](docs/phase-5-replay.md).
...
npm run compile:capability -- \
  ...
  --output capabilities/lookup-savings-balance.json \
  --input member_id=10001
```

With the app running, replay the generated artifact for a different member without an OpenAI key:

```bash
env -u OPENAI_API_KEY npm run replay -- \
  --artifact capabilities/lookup-savings-balance.json \
  --input member_id=10002 \
  --log evidence/replay-member-10002.jsonl
```

Replay follows the artifact with Playwright, verifies each checkpoint, and returns a typed USD money value. The committed replay evidence records `decisionSource: "artifact"` and `modelCalls: 0` and redacts the runtime input and output.
